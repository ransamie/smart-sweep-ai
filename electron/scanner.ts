...
function getQuarantinePath(originalPath: string) {
...
export async function restoreFile(originalPath: string): Promise<void> {
  ...
  try {
    ...
    // Clean up metadata
    try { await fs.unlink(qPath + '.meta.json'); } catch(e) {}
  } catch (e) {
...
export async function scanDirectory(dirPath: string, isBackground = false): Promise<FileInfo[]> {
  ...
  async function processDir(currentPath: string): Promise<void> {
    ...
    for (const dir of dirEntries) {
      if (SKIP_DIRS.has(dir.name.toLowerCase())) continue;
...
export function getFileSummary(files: FileInfo[]) {
  let totalSize = 0;
